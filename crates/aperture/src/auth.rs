...
pub fn groups(headers: &HeaderMap) -> Vec<String> {
...
pub fn require_admin(headers: &HeaderMap) -> Result<(), crate::error::AppError> {
    if is_admin(headers) {
        Ok(())
    } else {
...
mod tests {
    ...
    #[test]
    fn groups_parse_and_require_admin_gates() {
        // No X-Auth-Groups -> no groups, not admin, require_admin rejects.
        let none = HeaderMap::new();
        assert!(groups(&none).is_empty());
