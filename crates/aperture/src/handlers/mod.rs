...
pub const APP_CSS_PATH: &str = "/assets/aperture-20260909.css";
...
/// Render Aperture's chrome: the Scriptoria suite bar (six vhosts, Drive filled) over a section
/// bar carrying Drive's own pages. `active` names the current page; `email` is the gateway
/// identity, `None` on a public capability page.
pub fn topbar(active: &str, email: Option<&str>) -> String {
    let here = match active {
        "Requests" => "requests",
        "Drive admin" => "admin",
        _ => "files",
    };
    let mut nav = String::from(r#"<nav class="appbar__nav" aria-label="Drive pages">"#);
    ...
    for (href, label, key) in items {
        nav.push_str(&format!(
            r#"<a class="appnav{state}" href="{href}"{aria}>{label}</a>"#,
            state = if key == here { " is-active" } else { "" },
            href = href,
            aria = if key == here {
                r#" aria-current="page""#
            } else {
...
pub fn render_error(
    ...
    let body = ERROR_HTML
        ...
        .replace("{{TOPBAR}}", &topbar("Drive", email))
...
pub const SURFACE_KEY: &str = "drive";
pub const SURFACE_HOST: &str = "drive.w33d.xyz";
...
pub fn suite_bar(email: &str, extra_right: &str) -> String {
  ...
  <a class="suitebar__brand" href="/" aria-label="Scriptoria Drive home">
...
pub const FOOTER: &str = r##"<footer class="v2-foot">
  <span class="v2-foot__lead">Steadholme · Scriptoria · drive.w33d.xyz</span>
