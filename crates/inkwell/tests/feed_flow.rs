...
async fn feeds_expose_published_posts_only() {
    ...
    assert!(ctype.starts_with("application/rss+xml"), "rss content-type, got {ctype}");
    assert!(body.contains("<rss version=\"2.0\""), "declares RSS 2.0");
    assert!(body.contains("<title>Published Post</title>"), "published post is in the feed");
    assert!(
        body.contains("<link>https://blog.w33d.xyz/p/published-post</link>"),
        "item carries an absolute reading-view link"
    );
    assert!(body.contains("<pubDate>"), "item carries an RFC 822 pubDate");
    assert!(!body.contains("Secret Draft"), "drafts are NEVER syndicated");

    // --- /sitemap.xml ------------------------------------------------------
    let (status, ctype, body) = call(&state, "/sitemap.xml").await;
    assert_eq!(status, StatusCode::OK);
    assert!(ctype.starts_with("application/xml"), "xml content-type, got {ctype}");
    ...
    assert!(!body.contains("secret-draft"), "drafts are NEVER in the sitemap");
    ...
    assert!(body.contains(r#"href="/feed.xml""#), "alternate link points at /feed.xml");
}
...
async fn empty_blog_feeds_are_well_formed() {
    ...
    assert_eq!(status, StatusCode::OK);
    assert!(body.contains("<channel>") && body.contains("</channel>"), "valid empty channel");
