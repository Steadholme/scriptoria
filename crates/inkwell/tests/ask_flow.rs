...
async fn ask_retrieves_with_citations_and_excludes_drafts() {
    ...
    create(&state, "Rust Async Runtime", "the tokio runtime drives async rust tasks on the gateway", true).await;
    create(&state, "Gateway Routing", "the gateway routes async rust requests through tokio", true).await;
    create(&state, "Sourdough Bread", "a recipe for baking bread with flour and water", true).await;
    create(&state, "Secret Async Notes", "draft about the async rust runtime and tokio gateway", false).await;

    // --- GET /ask renders + mints a CSRF cookie ----------------------------
    let resp = app(state.clone()).oneshot(get_auth("/ask", "u_alice", "alice@hf")).await.unwrap();
    ...
    let set_cookie = resp
        ...
        .unwrap_or("");
    assert!(set_cookie.contains("__Host-csrf="), "GET /ask mints CSRF cookie");
    ...
    let (status, _) = call(&state, post_csrf("/api/ask", &body, Some(("u_alice", "alice@hf")))).await;
    ...
    let (status, html) = call(&state, post_csrf("/api/ask", &body, Some(("u_alice", "alice@hf")))).await;
    assert_eq!(status, StatusCode::OK);
    assert!(html.contains("Answer"), "answer section rendered");
    assert!(html.contains("Sources"), "sources section rendered");
    assert!(html.contains(r#"href="/p/rust-async-runtime""#), "links the matching post");
    assert!(html.contains(r#"href="/p/gateway-routing""#), "links the second matching post");
    assert!(!html.contains("Sourdough Bread"), "off-topic post not cited");
    assert!(!html.contains("Secret Async Notes"), "DRAFT never surfaces in an answer");
...
async fn related_posts_block_links_siblings_only() {
    ...
    assert!(html.contains(r#"href="/p/gateway-routing""#), "links the related sibling");
    assert!(!html.contains("Sourdough Bread"), "off-topic post not related");
    assert!(!html.contains("Secret Async Notes"), "draft never appears as related");
    // The article must not list ITSELF as related.
    let related_section = html.split("Related posts").nth(1).unwrap_or("");
    assert!(!related_section.contains(r#"href="/p/rust-async-runtime""#), "post is not its own related");
}
