//! Inkwell entry point: init state from env (select store, migrate), serve.
//!
//! Also exposes a dependency-free `inkwell healthcheck` subcommand used as the container
...
async fn main() {
    ...
    let state = match inkwell::build_state_from_env().await {
        Ok(state) => state,
        Err(e) => {
    ...
    let app = inkwell::app(state);

    let listener = tokio::net::TcpListener::bind(addr)
        .await
        .unwrap_or_else(|e| panic!("failed to bind {addr}: {e}"));

    tracing::info!(%addr, "Inkwell listening (personal blog / CMS)");
    axum::serve(listener, app).await.expect("server error");
}

/// GET `/healthz` over a raw TCP socket. Returns process exit code (0 = healthy).
fn run_healthcheck() -> i32 {
    let bind_addr = std::env::var("BIND_ADDR").unwrap_or_else(|_| "127.0.0.1:8700".to_string());
