...
//!
//! Inkwell only carries the logical fields (actor / action / target=post-slug / severity /
//! detail). Post bodies never ride an event — `detail` stays short and value-free.
...
mod tests {
    ...
    fn disabled_sink_is_noop_and_never_drops() {
        ...
        for _ in 0..1000 {
            sink.emit(AuditEvent::info("inkwell.post.create", "a@b", "post/x", "ok"));
        }
    ...
    /// An event serializes to exactly the safe shared fields, stamped with this service's source.
    #[test]
    fn event_serializes_to_safe_fields() {
        let ev = AuditEvent::notice("inkwell.post.delete", "alice@w33d.xyz", "my-post", "delete");
        ...
        assert_eq!(v["action"], "inkwell.post.delete");
        assert_eq!(v["source"], "inkwell");
    }
    ...
    async fn emit_never_blocks_when_sink_unreachable() {
        ...
        for _ in 0..(QUEUE_CAPACITY * 8) {
            sink.emit(AuditEvent::info("inkwell.post.create", "u", "post/x", "v1"));
        }
        assert!(
            sink.dropped() > 0,
