...
//!
//! Lattice only carries the logical fields (actor / action / target=page-slug / severity /
//! detail). Page bodies never ride an event — `detail` stays short and value-free.
...
mod tests {
    ...
    fn disabled_sink_is_noop_and_never_drops() {
        ...
        for _ in 0..1000 {
            sink.emit(AuditEvent::info("lattice.page.save", "a@b", "page/x", "ok"));
        }
    ...
    fn event_serializes_to_safe_fields() {
        let ev = AuditEvent::notice("lattice.page.save", "alice@w33d.xyz", "runbook", "edit");
        ...
        assert_eq!(v["action"], "lattice.page.save");
        assert_eq!(v["source"], "lattice");
    }
    ...
    async fn emit_never_blocks_when_sink_unreachable() {
        ...
        for _ in 0..(QUEUE_CAPACITY * 8) {
            sink.emit(AuditEvent::info("lattice.page.save", "u", "page/x", "v1"));
        }
        assert!(
            sink.dropped() > 0,
