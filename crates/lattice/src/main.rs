//! Lattice entry point: init state from env (select store, migrate), serve.
//!
//! Also exposes a dependency-free `lattice healthcheck` subcommand used as the container
...
async fn main() {
    ...
    let state = match lattice::build_state_from_env().await {
        Ok(state) => state,
        Err(e) => {
    ...
    let app = lattice::app(state);

    let listener = tokio::net::TcpListener::bind(addr)
        .await
        .unwrap_or_else(|e| panic!("failed to bind {addr}: {e}"));

    tracing::info!(%addr, "Lattice listening (knowledge-base wiki)");
    axum::serve(listener, app).await.expect("server error");
}

/// GET `/healthz` over a raw TCP socket. Returns process exit code (0 = healthy).
fn run_healthcheck() -> i32 {
    let bind_addr = std::env::var("BIND_ADDR").unwrap_or_else(|_| "127.0.0.1:8720".to_string());
    // Healthcheck always talks to the loopback regardless of the bind interface.
    let port = bind_addr.rsplit(':').next().unwrap_or("8720");
