...
//!
//! Echo only carries the logical fields (actor / action / target=thread-or-comment / severity /
//! detail). Comment bodies never ride an event — `detail` stays short and value-free.
...
mod tests {
    ...
    fn event_serializes_to_safe_fields() {
        let ev = AuditEvent::notice("echo.moderate", "alice@w33d.xyz", "cmt_123", "hide");
        ...
        assert_eq!(v["action"], "echo.moderate");
        assert_eq!(v["source"], "pastefire");
    }
    ...
    async fn emit_never_blocks_when_sink_unreachable() {
        ...
        for _ in 0..(QUEUE_CAPACITY * 8) {
            sink.emit(AuditEvent::info("echo.comment.post", "u", "thread/x", "v1"));
        }
        assert!(
            sink.dropped() > 0,
